...
from kitchen_shift.kitchen_v1 import Kitchen_v1
from constants import OBS_ELEMENT_INDICES, OBS_ELEMENT_GOALS, FRANKA_INIT_QPOS

PARAMS = dict(kettle_positions = ['top_right_angled_randomized',
                                  'bot_right_angled_randomized',
                                  'bot_left_angled_randomized'],
...
class MujocoWorldgenKitchenEnvWrapper(Kitchen_v1):
    """
    ...
    def sample_domain_parameters(self):
        domain_parameters = []
        domain_parameters.append(('change_microwave', self.np_random2.choice(4)))
        domain_parameters.append(('change_kettle', self.np_random2.choice(8)))
        domain_parameters.append(('change_objects_layout', 'microwave', 'randomized'))
        domain_parameters.append(('change_objects_layout', 'ovenhood', 'randomized'))
        domain_parameters.append(('change_objects_layout', 'kettle', 
                                  self.np_random2.choice(PARAMS['kettle_positions'])))
        ...
        domain_parameters.append(('change_counter_texture', 
    ...
    def _change_cabinet_texture(self, cabinet, cabinet_texture):
        ...
        p = os.path.join(os.getcwd(), 'assets', fn)
...
#                                             euler="0 0 {e}".format(e = 0.785 + self.np_random2.normal() * 0.05)),
                'randomized': dict(pos="{x} {y} 1.6".format(x = -0.800 + self.np_random2.normal() * 0.025, 
                                                           y = -0.200 + self.np_random2.normal() * 0.025),
                                   euler="0 0 {e}".format(e = 0.25 + self.np_random2.uniform() * 0.5)),
            },
            'hinge': {
            ...
            'ovenhood': {'right_raised': dict(pos="0.1 0 0.35"),
                         'center_raised': dict(pos="0 0 0"),
                         'randomized': dict(pos="{x} 0.0 0.0".format(x = self.np_random2.normal() * 0.05))}
        }
...
#                kettle_qpos = np.array([0.23, 0.75, 1.62]) + self.np_random2.normal() * 0.01
            if layout == 'top_right_angled_randomized':
                kettle_qpos = self.np_random2.normal(loc = [0.23, 0.75, 1.62], scale = [0.025, 0.025, 0.00])
                kettle_qrot = self.np_random2.normal(loc = [-0.99, 0.0, 0.0, -0.5236], scale = [0.0, 0.0, 0.0, 0.125])
            elif layout == 'bot_right_angled_randomized':
                kettle_qpos = self.np_random2.normal(loc = [0.23, 0.3504, 1.62], scale = [0.025, 0.025, 0.00])
                kettle_qrot = self.np_random2.normal(loc = [-0.99, 0.0, 0.0, -0.5236], scale = [0.0, 0.0, 0.0, 0.125])
    ...
    def perturb_camera_params(self):
        self.camera_params['distance'] = 2.5 + self.np_random2.normal(0.0, 0.05)
        self.camera_params['lookat'] = [-0.2, 0.5, 2.0] + self.np_random2.normal(0.0, 0.05, size=3)
        self.camera_params['azimuth'] = 70.0 + self.np_random2.normal(0.0, 1.0)
        self.camera_params['elevation'] = -50.0 + self.np_random2.normal(0.0, 1.0)
