...
PARAMS = dict(kettle_positions = dict(top_right_angled_randomized = [0.23, 0.75, 1.625],
...
class Randomized_Kitchen(Kitchen_v1):
    ...
    def sample_domain_parameters(self):
        ...
        microwave_params = dict(pos="{x} {y} 1.6".format(
        ...
        domain_parameters.append(('change_objects_layout', 'microwave', microwave_params))
        
        ovenhood_params = dict(pos="{x} 0.0 0.0".format(x = self.np_random2.normal() * 0.05))
        domain_parameters.append(('change_objects_layout', 'ovenhood', ovenhood_params))
        
        kettle_position = self.np_random2.choice(list(PARAMS['kettle_positions'].keys()));
        kettle_params = dict(kettle_qpos = self.np_random2.normal(loc = PARAMS['kettle_positions'][kettle_position], 
                                                                  scale = [0.025, 0.025, 0.00]),
                             kettle_qrot = self.np_random2.normal(loc = [-0.99, 0.0, 0.0, -0.5236], 
                                                                          scale = [0.0, 0.0, 0.0, 0.15]))

        domain_parameters.append(('change_objects_layout', 'kettle', kettle_params))
        ...
        domain_parameters.append(('change_robot_init_qpos', reset_qpos[: self.N_DOF_ROBOT]))
